...
def readMCData(ckt, ttype, tech, ccase='TT'):
    ...
    tech: 45, 32, 22, 16
    """
    variance = 'ProcessAndMismatch'
    lfname = '%s_%s_%d.mcdata' % (ckt, ttype, tech)
    lfullname = os.path.join(ckt, variance, lfname)
    ...
    for (v, f_3sigma, fmin, fmean, f_2sigma, f_sigma) in zip(vdd, freq_3sigma, freq_min, freq_mean, freq_2sigma, freq_sigma):
        ...
        ols.append(ol)
    ofn = os.path.join(ckt, variance, 'new_%s_%s_%d.mcdata'%(ckt, ttype,tech))
...
for ckt in ('adder4', 'adder8','adder16'):

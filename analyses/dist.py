...
FIG_DIR,DATA_DIR = analysis.make_ws('gaussian')
...
class Worker(multiprocessing.Process):
    ...
    def process(self, job):
        ...
        cov_list = scipy.stats.uniform.rvs(0.1, 0.7, size=100)
        kid = job
        for cov in cov_list:
            app = App(f=1)
        ...
        return (fpga_area_list,cov_list)
...
class FPGAAnalysis(BaseAnalysis):
    ...
    def analyze(self):
        kernels = kernel.gen_kernel_gauss(80,10,100)
        ...
        for kid in kernels:
            area,cov = result_queue.get()
            fpga_area_lists.append(area)
    ...
    def plot(self):
        ...
        with open(dfn, 'rb') as f:
            ...
            for fpga_area_list,cov_list in zip(fpga_area_lists, cov_lists):
                axes.plot(cov_list, fpga_area_list, 'o', color='blue')

            axes.set_xlabel('Kernel coverage (%)')
            axes.set_ylabel('Optimal FPGA allocation (%)')

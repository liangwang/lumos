...
if not args.kernels:
    kernels = ['fe', 'fd', 'stemmer', 'crf', 'dnn-asr', 'gmm', 'regex',
               'coreonly']
else:
...
for n in range(args.napps):
    rand_indexes = sorted([random.randrange(randrange_min, randrange_max) for _
                           in range(nkernels - 1)])
    ...
    for idx, k_ in enumerate(kernels):
        cov = (
            rand_indexes[idx + 1] - rand_indexes[idx]) / 10 ** args.precision
